...
async def is_equiv_sympy(x1: str, x2: str) -> bool:
    ...
    try:
        with timeout(seconds=5):
            # Try sympify first (handles plain expressions like "75", "1/2", "x^2+1")
            try:
                parsed_x1 = sympy.sympify(x1, evaluate=False)
                parsed_x2 = sympy.sympify(x2, evaluate=False)
                logger.debug(f"Successfully parsed with sympify: {x1} and {x2}")
            except Exception as e:
                logger.debug(f"sympify failed, trying LaTeX parsing: {e}")
                # Fall back to LaTeX parsing for LaTeX-formatted expressions
                try:
                    from sympy.parsing.latex import parse_latex  # type: ignore
                    parsed_x1 = parse_latex(x1)
                    parsed_x2 = parse_latex(x2)
                    logger.debug(f"Successfully parsed with LaTeX: {x1} and {x2}")
                except Exception as latex_e:
                    logger.debug(f"Couldn't parse one of {x1} or {x2}: {latex_e}")
                    return False
