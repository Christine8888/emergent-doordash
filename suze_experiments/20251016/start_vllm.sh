...
# Also kill any stray vllm or load balancer processes (multiple patterns to catch all)
...
for i in $(seq 0 $((NUM_INSTANCES - 1))); do
    ...
    # For base models without chat template, provide a simple one
    # Check if this is a base model (non-chat/instruct)
    EXTRA_ARGS=()
    if [[ "$MODEL_NAME" != *"-it"* ]] && [[ "$MODEL_NAME" != *"Instruct"* ]] && [[ "$MODEL_NAME" != *"Chat"* ]]; then
        echo "Base model detected, adding simple chat template"
        EXTRA_ARGS+=(--chat-template '{% for message in messages %}{{ message.content }}{% endfor %}')
    fi

    echo "Launching vLLM server for $MODEL_NAME on port $PORT"
    CUDA_VISIBLE_DEVICES=$CUDA_DEVICES vllm serve "$MODEL_PATH" "${VLLM_ARGS[@]}" "${EXTRA_ARGS[@]}" --port $PORT &
    VLLM_PIDS+=($!)
    sleep 5
