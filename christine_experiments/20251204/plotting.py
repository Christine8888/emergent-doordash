...
from src.modelx import size, clean_name, model_eci, fit_sigmoid, fit_joint_sigmoid, format_equation
...
def plot_by_x_axis(
    ...
    """Plot accuracy vs configurable x-axis (model size or ECI).

    Args:
        df: DataFrame with columns: model, hint, accuracy, stderr (optional)
        x_axis: What to plot on x-axis:
            - "model_size": Model size in billions (from size())
            - "eci": Epoch Capabilities Index (from model_eci())
        title: Plot title
        figsize: Figure size
        fit_sigmoid_curves: If True, fit per-hint sigmoids
        fit_scaling: If True, fit y = h·σ(...), else y = σ(...)
        fit_models: Models to use for fitting:
            - None: use all models
    ...
    # Build (model, x_value) pairs, filtering out None
    model_x_values = []
    for m in models:
        x_val = get_x_value(m)
        if x_val is not None:
            model_x_values.append((m, x_val))
        else:
            logger.warning(f"Could not get {x_axis} for model: {m}")

    model_x_values = sorted(model_x_values, key=lambda x: x[1])
    ...
    if fit_models is None:
        ...
        fit_model_names = set([m for m, _ in model_x_values[:fit_models]])
    else:
        fit_model_names = set(fit_models)
    ...
    # Fit joint model if requested (using modelx.fit_joint_sigmoid)
    joint_fit = None
    if fit_joint:
        try:
            joint_fit = fit_joint_sigmoid(
                ...
                hint_transform=hint_transform,
                exclude_hints=exclude_set,
                fit_models=fit_model_names,
                use_log_x=(xscale == "log"),
                x_values=get_x_value,  # Pass the getter function
            )
            params = joint_fit["params"]
            if include_cross:
                print(f"Joint fit: α={params[0]:.3f}, β={params[1]:.3f}, γ={params[2]:.3f}, δ={params[3]:.3f}")
    ...
    for hint_idx, hint in enumerate(hints):
        ...
        if fit_sigmoid_curves and len(x_vals_fit) >= min_points:
            try:
                fit_result = fit_sigmoid(
                    np.array(x_vals_fit), np.array(y_vals_fit),
                    use_log=(xscale == "log"), scale=fit_scaling,
                )
                hint_fits[hint] = fit_result
                ...
                if xscale == "log":
                    ...
                    x_smooth = np.linspace(min(x_vals) - 0.05 * x_range, max(x_vals) + 0.05 * x_range, 100)
                y_smooth = fit_result["predict"](x_smooth)
                ax.plot(x_smooth, y_smooth, color=color, linestyle="-", linewidth=2, alpha=0.6)
            except Exception as e:
                print(f"Warning: Failed to fit sigmoid for hint={hint}: {e}")
    ...
    #
    # if joint_fit is not None:
    #     H_label = "H'" if use_transform else "H"
    #     params = joint_fit["params"]
    #     if joint_fit["include_cross"]:
    #         joint_eq = f"σ({params[0]:.2f}C {params[1]:+.2f}{H_label} {params[2]:+.2f}C{H_label} {params[3]:+.2f})"
    #     else:
    #         joint_eq = f"σ({params[0]:.2f}C {params[1]:+.2f}{H_label} {params[2]:+.2f})"
    #     hint_handles.append(Line2D([0], [0], color="gray", linestyle="--", linewidth=2))
    #     hint_labels.append(joint_eq)
    #
    # ax.legend(hint_handles, hint_labels, title="hint fraction", framealpha=0.9, loc="upper left")
